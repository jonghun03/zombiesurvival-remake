...
function meta:ResetSpeed(noset, health)
	...
	if !(self.redeemer == nil) and (self.redeemer == true) then
		local rspeed = self.rspd
		local nspeed = 225
		if wep.GetWalkSpeed then
			local wspeed = wep:GetWalkSpeed()
			if rspeed > nspeed then
				if wspeed > nspeed then
					speed = rspeed + (wspeed * 0.45)
				elseif wspeed < nspeed then
					speed = rspeed - (wspeed * 1.75)
				else
					speed = rspeed
				end
			else
				if wspeed > nspeed then
					speed = rspeed + (wspeed * 0.95)
				elseif wspeed < nspeed then
					speed = rspeed - (wspeed * 0.15)
				else
					speed = rspeed
				end
			end
