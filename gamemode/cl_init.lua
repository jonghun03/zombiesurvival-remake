	...
	if obsmode == OBS_MODE_CHASE then
		...
		if target and target:IsValid() then
			...
			if target.IsCreeperNest or target.MinionSpawn then
				local txt = target.IsCreeperNest and "둥지" or "인형"
...
function GM:DrawCrateIndicators()
	...
	for i, arsenal in pairs(GAMEMODE.CachedArsenalEntities) do
		...
		if (distance >= 6400 and distance <= 1048576) and (not deployable or not WorldVisible(eyepos, pos)) then -- Limited to Scavenger's Eyes distance.
			...
			draw_SimpleTextBlurry("상점 상자", "ZS3D2DFont2Big", 0, 128, COLOR_GRAY, TEXT_ALIGN_CENTER)
...
function GM:DrawRemantlerIndicators()
	...
	for i, remantler in pairs(GAMEMODE.CachedRemantlerEntities) do
		...
		if (distance >= 6400 and distance <= 1048576) and (not deployable or not WorldVisible(eyepos, pos)) then -- Limited to Scavenger's Eyes distance.
			...
			draw_SimpleTextBlurry("무기 정제기", "ZS3D2DFont2Big", 0, 128, COLOR_GRAY, TEXT_ALIGN_CENTER)
...
function GM:DrawNestIndicators()
	...
	for i, nest in pairs(GAMEMODE.CachedNests) do
		...
		draw_SimpleTextBlurry("둥지", "ZS3D2DFont2Big", 0, 128, COLOR_GRAY, TEXT_ALIGN_CENTER)
...
local nanumgothic = "NanumGothic"
local fontfamily = "NanumGothic"
local fontfamilysm = "NanumGothic"
local fontfamily3d = "NanumGothic"
local ammofont = "KoreanNSR"
local fontsizeadd = 0
local fontweight = 0
local fontaa = true
...
function GM:CreateNonScaleFonts()
	surface.CreateLegacyFont(fontfamily, 96, 1000, true, false, "zshintfont", false, true)

	-- Default, DefaultBold, DefaultSmall, etc. were changed when gmod13 hit. These are renamed fonts that have the old values.
	surface.CreateFont("DefaultFontVerySmall", {font = fontfamily, size = 10, weight = 0, antialias = false, extended = true})
	surface.CreateFont("DefaultFontSmall", {font = fontfamily, size = 11, weight = 0, antialias = false})
	surface.CreateFont("DefaultFontSmallDropShadow", {font = fontfamily, size = 11, weight = 0, shadow = true, antialias = false, extended = true})
	surface.CreateFont("DefaultFont", {font = fontfamily, size = 13, weight = 500, antialias = false, extended = true})
	surface.CreateFont("DefaultFontAA", {font = fontfamily, size = 13, weight = 500, antialias = true, extended = true})
	surface.CreateFont("DefaultFontBold", {font = fontfamily, size = 13, weight = 1000, antialias = false, extended = true})
	surface.CreateFont("DefaultFontLarge", {font = fontfamily, size = 16, weight = 0, antialias = false, extended = true})
	surface.CreateFont("DefaultFontLargeAA", {font = fontfamily, size = 16, weight = 0, antialias = true, extended = true})
	surface.CreateFont("DefaultFontLargest", {font = fontfamily, size = 22, weight = 0, antialias = false, extended = true})
	surface.CreateFont("DefaultFontLargestAA", {font = fontfamily, size = 22, weight = 0, antialias = true, extended = true})
end
...
function GM:CreateScalingFonts()
	...
	surface.CreateLegacyFont("AYusungl", screenscale * (16 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontTiny", fontshadow, fontoutline)
	surface.CreateLegacyFont("AYusungl", screenscale * (20 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontSmallest", fontshadow, fontoutline)
	surface.CreateLegacyFont("AYusungl", screenscale * (22 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontSmaller", fontshadow, fontoutline)
	surface.CreateLegacyFont("AYusungl", screenscale * (28 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontSmall", fontshadow, fontoutline)
	surface.CreateLegacyFont("AYusungm", screenscale * (42 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFont", fontshadow, fontoutline)
	surface.CreateLegacyFont("AYusungb", screenscale * (72 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontBig", fontshadow, fontoutline)
	surface.CreateLegacyFont("AYusungm", screenscale * (16 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontTinyBlur", false, false, 8)
	surface.CreateLegacyFont("AYusungl", screenscale * (22 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontSmallerBlur", false, false, 8)
	surface.CreateLegacyFont("AYusungl", screenscale * (28 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontSmallBlur", false, false, 8)
	surface.CreateLegacyFont("AYusungm", screenscale * (42 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontBlur", false, false, 8)
	surface.CreateLegacyFont("AYusungm", screenscale * (72 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFontBigBlur", false, false, 8)
	surface.CreateLegacyFont(ammofont, screenscale * 35, fontweight, fontaa, false, "ZSHUDFontNaksue", fontoutline, fontoutline)
	surface.CreateLegacyFont("AHaetal", screenscale * 42, fontweight, fontaa, false, "ZSHUDFontDeath", fontoutline, fontoutline)
	...
	surface.CreateLegacyFont(fontfamily, 18 * liscreenscale, 0, true, false, "ZSScoreBoardPlayerSmall", false, true)
	surface.CreateLegacyFont(fontfamily, 15 * liscreenscale, 0, true, false, "ZSScoreBoardPlayerSmaller", false, true)
	surface.CreateLegacyFont(fontfamily, 11 * liscreenscale, 0, true, false, "ZSScoreBoardPing")
	...
	surface.CreateFont("ZSXPBar", {font = fontfamily, size = screenscale * 14, weight = 500, antialias = true, shadow = true, extended = true})
end
...
function GM:LastHumanMessage()
	...
	if P_Team(MySelf) == TEAM_UNDEAD or not MySelf:Alive() then
		self:CenterNotify({killicon = icon}, {font = "ZSHUDFontNaksue"}, " ", COLOR_RED, translate.Get(self.PantsMode and "kick_the_last_human" or "kill_the_last_human"), {killicon = icon})
	else
		self:CenterNotify({font = "ZSHUDFontNaksue"}, " ", COLOR_RED, translate.Get("you_are_the_last_human"))
...
function GM:HumanMenu()
	...
	local selecteditemtitle = EasyLabel(panel, "선택된 아이템", "ZSHUDFontSmall", color_white)
	selecteditemtitle:SetContentAlignment(5)
	panel:AddItem(selecteditemtitle)

	local selecteditemlabel = EasyLabel(panel, "주먹", "ZSHUDFontSmaller", color_white)
...
function GM:LocalPlayerDied(attackername)
	...
	if attackername then
		self:CenterNotify(COLOR_RED, {font = "ZSHUDFontDeath"}, translate.Get("you_have_died"))
