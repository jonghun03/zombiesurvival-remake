...
net.Receive("zs_boss_spawned", function(length)
	...
	elseif ent:IsValid() and P_Team(MySelf) == TEAM_UNDEAD then
		...
		GAMEMODE:CenterNotify(kid, " ", COLOR_RED, translate.Format("x_has_risen", translate.Get(classtbl.TranslationName)), kid)
	end
...
net.Receive("zs_boss_slain", function(length)
	...
	if ent:IsValid() then
		GAMEMODE:CenterNotify(ki, " ", COLOR_YELLOW, translate.Format("x_has_been_slain_as_y", translate.Get(classtbl.TranslationName), ent:Name()), ki)
	end
...
net.Receive("zs_ammopickup", function(length)
	...
	GAMEMODE:CenterNotify({killicon = ico}, " ", COLOR_GREEN, translate.Format("obtained_x_y_ammo", ammotype, amount))
end)
...
net.Receive("zs_ammogive", function(length)
	...
	GAMEMODE:CenterNotify({killicon = ico}, " ", COLOR_GREEN, translate.Format("gave_x_y_ammo_to_z", ammotype, amount, ent:Name()))
end)
...
net.Receive("zs_ammogiven", function(length)
	...
	GAMEMODE:CenterNotify({killicon = ico}, " ", COLOR_GREEN, translate.Format("obtained_x_y_ammo_from_z", ammotype, amount, ent:Name()))
end)
