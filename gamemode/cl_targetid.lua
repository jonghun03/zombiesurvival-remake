...
function GM:DrawTargetID(ent, fade)
	...
	if ent:Team() == TEAM_UNDEAD then
		...
		if holding:IsValid() then
			draw.SimpleTextBlur(string_format("[%s] 옮기는 중..", string_match(holding:GetModel(), ".*/(.+)%.mdl") or "object"), "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
		else
...
function GM:DrawSigilTargetHint(ent, fade)
	...
	draw.SimpleTextBlur("장치", "ZSHUDFontSmaller", x, y, colTemp, TEXT_ALIGN_CENTER)
	y = y + draw.GetFontHeight("ZSHUDFontSmaller") + 0

	draw.SimpleTextBlur("E키를 누르면 텔레포트", "ZSHUDFontTiny", x, y, colTemp, TEXT_ALIGN_CENTER)
end
