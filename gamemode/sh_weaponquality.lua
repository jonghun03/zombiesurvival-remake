...
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MIN_SPREAD, 				"최소 탄퍼짐", 			false, 	{ConeMin = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MAX_SPREAD,					"최대 탄퍼짐", 			false, 	{ConeMax = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_FIRE_DELAY,					"발사 딜레이", 				false, 	{Delay = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_RELOAD_SPEED,				"재장전 속도",		 		false, 	{ReloadSpeed = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_CLIP_SIZE,					"장탄수", 				true, 	{ClipSize = true}).ReqClip = true
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_RANGE,				"공격 범위", 				false, 	{MeleeRange = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_SIZE,					"무기 크기", 				false, 	{MeleeSize = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_IMPACT_DELAY,			"근접 공격 딜레이", 		false, 	{SwingTime = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_PROJECTILE_VELOCITY,		"투사체 속도", 		false, 	{ProjVelocity = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_SHORT_TEAM_HEAT,			"발열량", 			false, 	{HeatBuildShort = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_SHOT_COUNT,					"발사하는 탄 갯수", 				true, 	{NumShots = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_BULLET_PIERCES,				"총알 관통력", 			true, 	{Pierces = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MAXIMUM_MINES,				"지뢰 최대 갯수", 			true, 	{MaxMines = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MAX_DISTANCE,				"최대 거리", 		false, 	{MaxDistance = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_AURA_RADIUS,				"오오라 감지 범위", 	false, 	{AuraRange = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_RECOIL,						"반동", 					false, 	{Recoil = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_DAMAGE,						"공격력", 					false, 	{Damage = true, MeleeDamage = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEALRANGE,					"회복 범위", 				false, 	{HealRange = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEALCOOLDOWN,				"회복 딜레이", 		false, 	{Delay = true})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_BUFF_DURATION,				"버프 지속시간", 			false, 	{BuffDuration = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_LEG_DAMAGE,					"슬로우 디버프 강도", 			false, 	{LegDamage = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_REPAIR,						"수리량", 			false, 	{Repair = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_TURRET_SPREAD,				"터렛 탄퍼짐", 	false, 	{TurretSpread = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEALING,					"회복량", 			false, 	{Heal = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_HEADSHOT_MULTI,				"헤드샷 데미지 보너스", 	false, 	{HeadshotMulti = false})
GM:AddWeaponQualityModifier(WEAPON_MODIFIER_MELEE_KNOCK,				"넉백", 				false, 	{MeleeKnockBack = false})
...
function GM:CreateWeaponOfQuality(i, orig, quality, classname, branch)
	...
	if wept.PrimaryRemantleModifier then
		...
		table.insert(remantledescs, "+" .. ((quality[2]-1) * 100) .. "% " .. "데미지")
	end
