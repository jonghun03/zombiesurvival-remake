...
net.Receive("zs_skills_nextreset", function(length)
	...
	if GAMEMODE.SkillWeb and GAMEMODE.SkillWeb:IsValid() then
		...
		btn:SetText(GAMEMODE.NextSkillReset <= 0 and "초기화" or (days .. " 일 남음"))
...
local REMORT_SKILL = {Name = "환생", Description = "더 멀리 나아가세요.\n모든 스킬, 경험, 스킬 포인트, 레벨을 잃습니다.\n레벨 1부터 다시 시작하지만, 1개의 추가 스킬포인트를 얻습니다.\n여러 번 할 수 있으며 여러 번 추가 스킬 포인트를 얻을 수 있습니다."}
...
function PANEL:Init()
	...
	savebtn.DoClick = function(me)
		...
		local frame = Derma_StringRequest("스킬 조합 저장", "이 스킬 조합의 이름을 지정해주세요.", "이름",
		function(strTextOut)
			SaveSkillLoadout(strTextOut)
			UpdateDropDown(dropdown)

			self:DisplayMessage("스킬 조합 '" .. strTextOut .."'이 저장되었습니다!", COLOR_GREEN)
		end,
	...
	loadbtn.DoClick = function(me)
		...
		self:DisplayMessage("스킬 조합 '" .. nlname .."'을 불러왔습니다!", COLOR_GREEN)
	end
	...
	activateall.DoClick = function(me)
		...
		if #MySelf:GetUnlockedSkills() == 0 then
			self:DisplayMessage("활성화 할 수 있는 스킬이 없습니다!", COLOR_RED)
		else
			self:DisplayMessage("모든 스킬이 활성화 되었습니다.", COLOR_GREEN)
		end
	...
	deactivateall.DoClick = function(me)
		...
		if #MySelf:GetUnlockedSkills() == 0 then
			self:DisplayMessage("끌 수 있는 스킬이 없습니다!", COLOR_RED)
		else
			self:DisplayMessage("습득한 모든 스킬이 비활성화 되었습니다.", COLOR_RED)
		end
	...
	reset:SetText(resettime <= 0 and "초기화" or (days .. "일 남음"))
	...
	spremaining:SetChangeFunction(function()
		return "사용하지 않은 스킬 포인트: "..MySelf:GetZSSPRemaining()
	end, true)
	...
	button.DoClick = function(me)
		...
		if MySelf:IsSkillDesired(skillid) then
			...
			self:DisplayMessage(name.." 비활성화됨.")
		elseif MySelf:IsSkillUnlocked(skillid) then
			...
			self:DisplayMessage(name.." 활성화됨.", COLOR_DARKGREEN)
		else
			...
			self:DisplayMessage(name.." 습득과 함께 활성화됨!", COLOR_GREEN)
		end
	...
	warningtext:SetText("리스폰 후 변경 사항이 적용됩니다!")
...
function PANEL:UpdateQuickStats()
	...
	for i=1,3 do
		local prefix = i == 1 and "체력" or i == 2 and "속도" or "자금"
	...
	if mc == MOUSE_LEFT then
		...
		if hoveredskill then
			...
			if hoveredskill == -1 and can_remort then
				Derma_Query(
					"정말로 환생하기를 원하시나요?\n레벨 1로 되돌아 갈 것이고, 모든 스킬을 잊게 됩니다. 하지만 추가 SP 1을 드립니다.\n절대 되돌아 갈 수 없습니다!",
					"경고",
					"네",
			...
			elseif hoveredskill == -1 then
				self:DisplayMessage("환생을 하려면 50레벨이 되어야 합니다!", COLOR_RED)
				surface.PlaySound("buttons/button8.wav")

				return
			elseif MySelf:IsSkillDesired(hoveredskill) then
				if GAMEMODE.Skills[hoveredskill].AlwaysActive then
					self:DisplayMessage("이 스킬을 비활성화 할 수 없습니다!", COLOR_RED)
					surface.PlaySound("buttons/button8.wav")

					return
				end

				contextmenu.Button:SetText("비활성화")
			elseif MySelf:IsSkillUnlocked(hoveredskill) then
				contextmenu.Button:SetText("활성화")
			elseif MySelf:SkillCanUnlock(hoveredskill) then
				if MySelf:GetZSSPRemaining() >= 1 then
					contextmenu.Button:SetText("습득")
				else
					self:DisplayMessage("이 스킬을 습득하기 위해선 SP가 더 필요합니다!", COLOR_RED)
					surface.PlaySound("buttons/button8.wav")

					return
				end
			else
				self:DisplayMessage("이 스킬을 습득하기 위해선 이전 단계의 스킬을 배워야 합니다!", COLOR_RED)
				surface.PlaySound("buttons/button8.wav")
