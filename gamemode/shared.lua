...
function GM:GetNumberOfWaves()
	...
	local num = GetGlobalInt("numwaves", default) -- 이것은 logic_waves에 의해 제어됩니다
...
function GM:RefreshMapIsObjective()
	...
	elseif SERVER and (Config.NoSigilMLTMap) and (string.find(mapname, "_mlt_", 1, true)) then
		self.NoSigil = true
	elseif SERVER and table.HasValue(Config.NoSigilMap, mapname) then
		self.NoSigil = true
	else
...
function GM:OnPlayerHitGround(pl, inwater, hitfloater, speed)
	...
	if isundead then
		...
		recovery_mul = 1
		damage_mul = 1
	elseif !(pl.redeemer == nil) and (pl.redeemer == true) then
		speed = math.max(0, speed - 300)
		threshold_mul = 0
		slowdown_mul = 0
		recovery_mul = 0
		damage_mul = 0.25
