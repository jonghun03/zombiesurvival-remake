...
AddCSLuaFile("vgui/pfuckselect.lua")
...
weaponmodelstoweapon["models/weapons/w_knife_t.mdl"] = "weapon_zs_swissarmyknife2"
weaponmodelstoweapon["models/weapons/w_knife_ct.mdl"] = "weapon_zs_swissarmyknife2"
	...
	if NextTick <= time then
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				if !(pl.redeemer == nil) and (pl.redeemer == true) and !(pl.redeemregen == nil) and (pl.redeemregen == true) and (time >= pl.NextRegenRedeem) and ( pl:Health() < math.min( healmax, pl:GetMaxHealth() ) ) then
					pl.NextRegenRedeem = time + pl.redeemregdelay
					pl:SetHealth(math.min(healmax, pl:Health() + pl.redeemregamt))
				end
		...
		if self:GetEscapeStage() == ESCAPESTAGE_DEATH then
			for _, pl in pairs(allplayers) do
				if P_Team(pl) == TEAM_HUMAN then
					pl:TakeSpecialDamage(200, DMG_ACID) // 탈출 라운드 데미지
				end
			end
		end
	end
...
function GM:PlayerInitialSpawnRound(pl)
	...
	elseif self:GetNumberOfWaves() == -1 or self.NoNewHumansWave <= self:GetWave() or team.NumPlayers(TEAM_UNDEAD) == 0 and 1 <= team.NumPlayers(TEAM_HUMAN) then -- 게임 도중에 참가했거나 좀비가 없거나 일부 사람이 죽었거나 마감 시간을 지났습니다.
		pl:ChangeTeam(TEAM_UNDEAD)
...
function GM:PlayerSpawn(pl)
	...
	elseif pl:Team() == TEAM_HUMAN then
		...
		pl:ResetSpeed()
		pl:ResetJumpPower()
		pl:SetCrouchedWalkSpeed(0.35)		--웅크리고 걷는 속도(기본)
		pl:SetMaxHealth(Config.DefaultHealth)				--게임 기본 체력
		pl:SetHealth(Config.DefaultHealth)
		...
		if self.ZombieEscape then
			...
			pl:Give("weapon_zs_fists")		--게임시작지 지급되는 무기
			...
			elseif pl.m_PreRedeem then
				if self.RedeemLoadout then
					...
					local RTable = Config.RedeemerClass[r]
					for k, v in ipairs(RTable.Weapons) do
						pl:Give(v)
					end
					pl.redeemer = true
					pl:SetCrouchedWalkSpeed(0.9)																--웅크리고 걷는 속도
					pl:SetMaxHealth(pl:GetMaxHealth() + RTable.Health) --최대 체력
					pl:SetHealth(pl:Health() + RTable.Health) -- 체력
					if CLIENT then
						pl:SetMaxClientSpeed(RTable.MaxSpeed)
					end
					pl:SetModel(RTable.Model)
					pl.redeemregen = RTable.Regenerate
					pl.redeemregdelay = RTable.RegenDelay
					pl.redeemregamt = RTable.RegenHealth
					pl.rspd = RTable.MaxSpeed
					pl.rjmp = RTable.JumpPower
					pl:ResetSpeed()
					pl:ResetJumpPower()
					pl:SetMaxSpeed(RTable.MaxSpeed)
				end
			end
		end
