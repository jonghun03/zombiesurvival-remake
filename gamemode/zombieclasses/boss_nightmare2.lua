CLASS.Name = "룸"
CLASS.TranslationName = "class_nightmare2"
CLASS.Description = "description_nightmare2"
CLASS.Help = "controls_nightmare2"
...
local function CreateBoneOffsets(pl)
	pl.m_NightmareBoneOffsetsNext = CurTime() + math.Rand(0.85, 0.0001)  --최대시간 / 최소 시간

	local offsets = {}
	local angs = {}
	for i=1, pl:GetBoneCount() - 1 do
		if math.random(3) == 3 then
			offsets[i] = VectorRand():GetNormalized() * math.Rand(45, 3) --최대 흔들기 / 최소 흔들기
		end
		if math.random(5) == 5 then
			angs[i] = Angle(math.Rand(-150, 150), math.Rand(-150, 150), math.Rand(-150, 150))  --꺾이는 정도
		end
	end
