...
concommand.Add("zsgiveweapon", function(sender, command, arguments)
	...
	local currentwep = sender:GetActiveWeapon()
	local cwep = sender:GetWeapon(currentwep:GetClass())
	if cwep.Ungiveable then
		GAMEMODE:ConCommandErrorMessage(sender, translate.ClientGet(sender, "cant_give_weapon"))
		return
	end
