	...
	if itemtab then
		...
		if self.StockLabel then
			...
			if stocks ~= self.m_LastStocks then
				...
				self.StockLabel:SetText(stocks.."개 남음")
...
local function ItemPanelDoClick(self)
	...
	ppurbl:SetText(price .. (self.NoPoints and "개의 고철" or " 포인트"))
...
function GM:AddShopItem(list, i, tab, issub, nopointshop)
	...
	if missing_skill then
		...
		pricelabel:SetText(price..(nopointshop and "개의 고철" or " 포인트"))
	end
	pricelabel:SizeToContents()
	pricelabel:AlignRight(alignri)

	if tab.MaxStock then
		local stocklabel = EasyLabel(itempan, tab.MaxStock.."개 남음", "ZSHUDFontTiny")
...
function GM:CreateItemInfoViewer(frame, propertysheet, topspace, bottomspace, menutype)
	...
	local namelab = EasyLabel(purchaseb, "구매", "ZSBodyTextFontBig", COLOR_WHITE)
	...
	namelab = EasyLabel(ammopb, "탄약", "ZSBodyTextFontBig", COLOR_WHITE)
	namelab:SetVisible(false)
...
function GM:OpenArsenalMenu()
	...
	local subtitle = EasyLabel(topspace, "좀비 대재앙에서 생존하기 위해 필요한 모든 것!", "ZSHUDFontTiny", COLOR_WHITE)
	...
	local wsb = EasyButton(topspace, "자금 메뉴", 8, 4)
	...
	local pointslabel = EasyLabel(bottomspace, "지불할 포인트: 0", "ZSHUDFontTiny", COLOR_GREEN)
		...
		if hasitems then
			...
			if usecats then
				...
				for i = ind, (trinkets and #subcats or 5) do
					...
					tbn = EasyButton(tabpane, trinkets and subcats[i] or ("티어 " .. i), 2, 8)
