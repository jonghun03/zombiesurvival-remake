...
function MakepEndBoard(winner)
	...
	if localwin then
		...
		heading = EasyLabel(frame, "승리!", "ZSHUDFontBig", COLOR_CYAN)
	else
		surface.PlaySound("ambient/levels/citadel/strange_talk"..math.random(3, 11)..".wav")
		heading = EasyLabel(frame, "패배", "ZSHUDFontBig", COLOR_RED)
	end
	...
	local subheading
	if localwin then
		subheading = EasyLabel(frame, "인간들은 살아남았습니다.", "ZSHUDFontSmaller", COLOR_WHITE)
	else
		subheading = EasyLabel(frame, "좀비 군단은 더욱 더 강해집니다.", "ZSHUDFontSmaller", COLOR_LIMEGREEN)
	end
...
function PANEL:SetPlayer(pl, col, misc, misccol, overridename)
	...
	if pl:IsValidPlayer() then
		...
		avatar:SetPlayer(pl)
		avatar:SetTooltip("이 곳을 클릭하면 프로필로 이동합니다.")
