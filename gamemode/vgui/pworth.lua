...
function MakepWorth()
	...
	local title = EasyLabel(topspace, "자본 상점", "ZSHUDFontSmall", COLOR_WHITE)
	title:CenterHorizontal()
	local subtitle = EasyLabel(topspace, "게임을 시작하면서 사용 할 아이템을 골라주세요.", "ZSHUDFontTiny", COLOR_WHITE)
	...
	local sheet = propertysheet:AddSheet("즐겨찾기", list, "icon16/heart.png", false, false)
	...
	local worthlab = EasyLabel(frame, "자금: "..tostring(remainingworth), "ZSHUDFontSmall", COLOR_LIMEGREEN)
	...
	if #GAMEMODE.SavedCarts == 0 then
		...
		propertysheet:SwitchToName("즐겨찾기")
	end
...
function PANEL:SetWorthID(id)
	...
	elseif tab.Price then
		self.PriceLabel:SetText(tostring(tab.Price).."원")
	else
...
function PANEL:DoClick(silent, force)
	...
	pWorth.WorthLab:SetText("자금: ".. remainingworth)
	if remainingworth <= 0 then
