...
local function DismantleClick()
	Derma_Query("무기를 분해하시겠습니까? 분해한 후 취소할 수 없습니다.", "분해 확인",
	"확인", function()
...
function PANEL:Paint(w, h)
	...
	if oldquality ~= hovquality or oldbranch ~= hovbranch then
		...
		if hovquality and hovbranch then
			...
			self.ScrapCost:SetText(scost ~= "" and "고철 사용량: " .. scost or "")
			...
			for i=1, 5 do
				dtxt = " "
				if txt ~= "기본" and altdesc and altdescs and altdescs[i] then
					dtxt = altdescs[i]
				end
...
net.Receive("zs_remantleconf", function()
	...
	local disscraptxt = gtbl.NoDismantle and "분해 불가" or "고철 " .. retscrap .. "개"
...
function GM:OpenRemantlerMenu(remantler)
	...
	local title = EasyLabel(topspace, "무기 정제기", "ZSHUDFontSmall", COLOR_WHITE)
	title:CenterHorizontal()
	local subtitle = EasyLabel(topspace, "무기를 고철로 갈고 그 고철로 다른 무기를 업그레이드 하세요!", "ZSHUDFontTiny", COLOR_WHITE)
	...
	local pointslabel = EasyLabel(bottomspace, "고철 보유량: 0", "ZSHUDFontTiny", COLOR_GREEN)
	pointslabel:AlignTop(4)
	pointslabel:AlignLeft(8)
	pointslabel.Think = ScrapLabelThink

	local lab = EasyLabel(bottomspace, "무기 분해는 취소할 수 없습니다!", "ZSHUDFontTiny")
	lab:AlignTop(4)
	lab:AlignRight(4)
	...
	local sheet = remprop:AddSheet("강화", remantleframe, "icon16/arrow_up.png", false, false)
	...
	sheet = remprop:AddSheet("장신구", trinketsframe, GAMEMODE.ItemCategoryIcons[ITEMCAT_TRINKETS], false, false)
	sheet.Panel:SetPos(0, tabhei + 2)
	...
	sheet = remprop:AddSheet("탄약", ammoframe, GAMEMODE.ItemCategoryIcons[ITEMCAT_AMMO], false, false)
	sheet.Panel:SetPos(0, tabhei + 2)
	...
	local contents = EasyLabel(remantleframe, gtbl and gtbl.PrintName or "비어있음", "ZSHUDFontSmall", COLOR_WHITE)
	...
	local disabtn = EasyButton(remantleframe, "무기 분해", 8, 4)
	...
	if gtbl then
		...
		disscraptxt = gtbl.NoDismantle and "분해 불가" or "고철 " .. retscrap .. " 개"
	end
