...
function SWEP:MeleeHitPlayer(ent, trace, damage, forcescale)
	ent:ThrowFromPositionSetZ(self:GetOwner():GetPos(), damage * 0.25 * (forcescale or self.MeleeForceScale))
	ent:MeleeViewPunch(damage * 0.25)
	local nearest = ent:NearestPoint(trace.StartPos)
	util.Blood(nearest, math.Rand(damage * 0.01, damage * 0.05), (nearest - trace.StartPos):GetNormalized(), math.Rand(damage * 0.1, damage * 0.5), true)
end
