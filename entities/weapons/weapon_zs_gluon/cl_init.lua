...
local function DrawHeatBar(self, x, y, wid, hei, is3d)
	...
	if self:GetGunState() == 2 then
		...
		if ((CurTime() * 4) % 2) > 1 then
			...
			draw.SimpleTextBlurry("냉각 중", is3d and "ZS3D2DFontSmaller" or "ZSHUDFontSmaller", x + wid/2, bary + hei * 0.15, colRed, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	end
end
...
function SWEP:Draw2DHUD()
	...
	draw.SimpleTextBlurry("온도", "ZSHUDFont", x + wid * 0.5, yy + hei * 0.45, colRed, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
...
function SWEP:Draw3DHUD(vm, pos, ang)
	...
	cam.Start3D2D(pos, ang, self.HUD3DScale / 2)
		...
		draw.SimpleTextBlurry("온도", "ZS3D2DFontSmall", x + wid * 0.5, y - hei * 1, colRed, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	cam.End3D2D()
end
