SWEP.PrintName = "게틀링건"
SWEP.Description = "달구는 데 시간이 좀 걸린다. 우클릭으로 발사 없이 회전시킬 수 있다(발사 준비)."
...
GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'시타델' 미니캐논", "한 번에 3발을 쏘지만 느려진다. 데미지와 정확도는 좋아진다", function(wept)
...
function SWEP:GetFireDelay()
	return self.BaseClass.GetFireDelay(self) - (self:GetSpool() * 0.01)
end
...
function SWEP:CheckSpool()
	if self:GetSpooling() then
		if not self:GetOwner():KeyDown(IN_ATTACK) and not self:GetOwner():KeyDown(IN_ATTACK2) then
			...
			self:GetOwner():ResetSpeed()
			self:SetNextPrimaryFire(CurTime() + 1)						--쏘고난후 잠시 쉬는 쿨타임
			self:EmitSound("ambient/machines/spindown.wav", 75, 150)
		else
			self:SetSpool(math.min(self:GetSpool() + FrameTime() * 0.08, 5))	--가속도/최대배수
		end

		self.ChargeSound:PlayEx(1, math.min(255, 65 + self:GetSpool() * 25))
	else
		self:SetSpool(math.max(0, self:GetSpool() - FrameTime() * 36))
		self.ChargeSound:Stop()
	end
end
