...
SWEP.Description = "덕트 테이프로 묶인 나무 판자 더미.\n주변에 프롭이 없을 때 바리케이드를 건설하기에 유용하다.\n고정시키려면 망치와 못이 필요하다."
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if ent:IsValid() then
			...
			ent:SetAngles(ang)
			if table.HasValue(Config.Supporter.Rank, user:GetUserGroup()) then
				ent:SetModel(self.PremiumModels[math.random(#self.PremiumModels)])
			else
